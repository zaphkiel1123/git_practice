...
# First run: parses data, saves cache
  python3 breakout_retrace_analysis.py /path/to/data/dir --no-browser

# Subsequent runs with different params: loads from cache instantly
  python3 breakout_retrace_analysis.py /path/to/data/dir --min-vol 1500 --min-retrace 0.6 --no-browser
  python3 breakout_retrace_analysis.py /path/to/data/dir --target-ticks 40 --stop-ticks 15 --no-browser
"""
...
def _cache_key(filepaths: list[str]) -> str:
    """Deterministic hash of file paths + their sizes/mtimes for cache invalidation."""
    h = hashlib.sha256()
...
def _cache_path(filepaths: list[str], cache_dir: str | None = None) -> Path:
    """Return the cache file path for a given set of input files."""
    key = _cache_key(filepaths)
    directory = Path(cache_dir) if cache_dir else Path(filepaths[0]).parent
    return directory / f".tick_cache_{key}.pkl"


def load_multiple(filepaths: list[str], cache_dir: str | None = None, no_cache: bool = False) -> pd.DataFrame:
    """Load and concatenate multiple .data files, with pickle caching."""
    cache_file = _cache_path(filepaths, cache_dir)

    if not no_cache and cache_file.exists():
        print(f'  Loading cached ticks from {cache_file} ...')
        with open(cache_file, 'rb') as f:
            df = pickle.load(f)
        print(f'  Loaded {len(df):,} ticks from cache')
        return df
    ...
    if not no_cache:
        ...
        print(f'  Cached parsed ticks to {cache_file}')
...
def run(
    ...
    ticks = load_multiple(filepaths, cache_dir=cache_dir, no_cache=no_cache)
...
Examples:
  ...
  %(prog)s 20250904.data --min-retrace 0.6 --target-ticks 40 -o report.html
    ...
    parser.add_argument('--cache-dir', default=None,
                        help='Directory to store cached parsed data (default: same as input)')
    parser.add_argument('--no-cache', action='store_true',
                        help='Force re-parse of data files, ignoring cache')
